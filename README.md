...
### 💬 微信式聊天
- **1:1 复刻微信界面**：绿色气泡、尖角、头像、时间分隔、输入栏
- **流式回复**：TA 一个字一个字地打出来，像真人打字
- **表情系统**：支持微信内置 emoji（如 `[捂脸]`、`[旺柴]`）和自定义表情包
- **记忆内联标记**：聊天中打 `[记住:xxx]` 让 TA 永久记住某件事
- **时间感知**：TA 知道现在是几点、星期几——深夜会困、饭点会问吃没吃
...
项目使用 **MockAdapter** 实现全离线测试——所有 LLM 环节（对话/解析/提炼/朋友圈生成）均由模拟适配器覆盖，不消耗 API token。
...
测试覆盖：
- 模型适配器（MockAdapter 流式输出）
- 对话引擎（分条 / 表情 / 落库 / 多轮历史串联）
- 记忆服务（写入 / 读取 / 衰减 / 矛盾覆盖 / 关键词检索）
- 人格解析（json 抽取 / 画像编解码 / 采样 / 兜底）
- 输出后处理（分条 / 表情命中 / 未命中保留 / 字数延迟）
- 主动性调度（作息配额 / 开放回路对账 / 预生成引擎）
- 微信 emoji 转换
- 社交服务（朋友圈发布 / 点赞感知）
- 聊天记录解析（本地正则 / LLM 兜底）
...
| B+C / MVP | 建号、微信聊天、五层记忆、画像编辑器、表情系统、备份 | ✅ |
| D / 主动性 | 活动调度、开放回路、本地通知、预生成主动消息 | ✅ |
| E / 社交 | 对外人格、朋友圈、点赞感知 | ✅ |
| F / 扩展 | 多人/群聊、iOS、云备份、Claude 适配器 | ⬜ 按需 |

---

## 已知问题与注意事项

- **真机验证**：所有 LLM 环节已被 MockAdapter 单测覆盖，但真实模型输出质量需配 key 后真机实测校准
- **通知**：国产 ROM 杀后台策略各异，到点通知的可靠性取决于系统对 `SCHEDULE_EXACT_ALARM` 的支持
- **图片/表情迁移**：备份 JSON 不含表情图片文件本身（只存路径），换机需另行迁移图片
- **master 渠道**：项目当前使用 Flutter master 渠道（SDK ^3.12），切换到 stable 后需移除 `dependency_overrides` 中的 pin
- **Gradle 缓存**：换机/换路径后若构建报陌生绝对路径，先 `flutter clean` + 删除 `android/.gradle` 和 `android/app/build`
- **国内用户**：`android/` 下已配置阿里云/腾讯云镜像加速 Gradle 和 Android SDK 下载
